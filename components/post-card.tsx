...
import { storage, database, ID, Permission, Role, Query, client, APPWRITE_ENDPOINT, APPWRITE_PROJECT } from "@/libs/AppWriteClient";
...
export default function PostCard({ doc }: Props) {
  ...
  const { openComments, commentsPostId, closeComments, openShare } = useUI();
  ...
  const toggleFollowAuthor = async () => {
    ...
    try {
      if (followDocId) {
        ...
        const compositeId = makeFollowDocId(user.$id, String(authorId));
        try {
  ...
  useEffect(() => {
    if (!dbId || !likeCol || !APPWRITE_ENDPOINT || !APPWRITE_PROJECT) return;
    if (typeof window === 'undefined') return;
    const postId = String((doc as any).$id);
    const channel = `databases.${dbId}.collections.${likeCol}.documents`;
    ...
    const trySubscribe = () => {
      if (cancelled) return;
      try {
        unsub = client.subscribe(channel, cb);
      } catch (e: any) {
        const msg = String(e?.message || e || '');
        const isConnecting = /CONNECTING|Still in CONNECTING state/i.test(msg);
        if (isConnecting && attempts < maxAttempts) {
          attempts += 1;
          timer = setTimeout(trySubscribe, delayMs);
          return;
        }
        // If other error or retries exhausted, swallow to avoid crashing UI
      }
    };

    // Defer first attempt a tick to avoid dev-mode double mount/WS race
    timer = setTimeout(trySubscribe, 0);
